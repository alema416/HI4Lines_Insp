...
def train(loader, model, criterion, criterion_ranking, optimizer, epoch, history, args, classnumber):
    ...
    focal_criterion = focal_loss(class_num=classnumber)
    model.train()
    ...
    for i, (input, target, idx, _) in enumerate(loader):
        ...
        if torch.isnan(output).any() or torch.isinf(output).any():
            print("? ERROR: Model output contains NaN or Inf.")
            exit(1)

        # Enable anomaly detection if debugging (remove later for speed)

...
from degirum_tools.ui_support import Progress, stdoutRedirector
...
class ObjectDetectionModelEvaluator(ModelEvaluatorBase):
    ...
    def evaluate(
        ...
        for image_number in tqdm(range(0, num_images), desc=f'reading ground truth'):
        ...
        with self.model:
            ...
            for image_number, predictions in tqdm(enumerate(self.model.predict_batch(sorted_path_list)), desc=f'running inference'):    
                if self.show_progress:
                    progress.step()
        ...
        split = input('split: ')
        if custom == True:
            with open(f'./prediction_live_{split}.json', "w") as f:
                json.dump(jdict, f, indent=4)
            pred = anno.loadRes(jdict)
        else:
            with open(f'./annotations_{split}_multiclass_False.json', "r") as f:
                jdict = json.load(f)  # Load the JSON file into `jdict`
            ...
            predictions = [
                {
                    ...
                    "bbox": ann["bbox"],
                    "score": ann.get("score", 1.0)  # Ensure a score exists
                }
                for ann in jdict["annotations"]
            ]
            print(len(predictions))
            # Pass only the extracted detection results
        
            pred = anno.loadRes(predictions)
        #pred = anno.loadRes(jdict["annotations"])
        #with stdoutRedirector():
        #pred = 
